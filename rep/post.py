...
def extract_choice_with_regex(text: str) -> str:
    ...
    # Regex to find a single uppercase letter within the <answer> tags.
    # It allows for whitespace and other characters after the letter.
    # r"<answer>"      : Matches the opening tag
    # r"\s*"           : Matches any whitespace
    # r"([A-Z])"       : Captures a single uppercase letter (our target)
    # r"[^<]*"         : Matches any characters except '<' until the end tag
    # r"</answer>"     : Matches the closing tag
...
def process_with_llm(
    ...
    # Low temperature for deterministic extraction, short max_tokens as output is small
    ...
    system_prompt = (
        ...
        # "It is a match if the final calculation result in the generated answer match with the ground truth."
...
if __name__ == "__main__":
    # --- Configuration ---
    # NOTE: Assuming 'Qwen2.5-14B-Instruct' is in the same parent directory as the script.
    ...
    INPUT_JSON_PATH = "/home/sora/llm/moe/output/deepseek_pre/raw_result_med.json"
    
    # Specify the path for the final output
    OUTPUT_JSON_PATH = "/home/sora/llm/moe/output/deepseek_pre/final_choice_med.json"
        ...
        if len(input_samples) == len(model_choices):
            ...
            for sample, choice in zip(input_samples, model_choices):
                final_results.append({
                    "index": i,
                    "gold": sample.get("gold", "N/A"), # Safely get the 'gold' field
                    "choice": choice,
